...
//JK 4/12/16: Adding IMburn/IMrun filenames
char *imburnfile;
char *imrunfile;
...
//AS: Fri Jan  8 11:39:21 EST 2016
...
static TiXmlElement *callprintacceptancerates (FILE * outto, int currentid);
...
scan_commandline (int argc, char *argv[], int curr_id)
{
  ...
  const char *sep = ",:";
  int ioption;
//JK 4/12/16: Adding flags for specific burn/run mode
  int Bf = 0, Lf = 0;
...
//the program hangs. Fixing this Thu Aug  4 13:40:59 EDT 2016
  ...
  if (Bf == 1) {
    char buf[ucbuffer];
    imburnfile = (char *) malloc(ucbuffer);
    strcpy(buf,outfilename);
    strcat(buf,".IMburn");
    strcpy(imburnfile,buf);
  }
  if (Lf == 1) {
    char buf[ucbuffer];
    imrunfile = (char *) malloc(ucbuffer);
    strcpy(buf,outfilename);
    strcat(buf,".IMrun");
    strcpy(imrunfile,buf);
  }
...
///AS: Ignoring assignment options for now
  ...
  if (!Hkp) // default is to swap a once step or once for every 10 chains each step whichver is greater
    //swaptries = numchains;
    //AS: Tue Mar  8 15:43:31 EST 2016
...
//AS: 3/6/2016
//Have to reassign the values on the head node to 0 before starting to count
/*#ifdef MPI_ENABLED
if (numprocesses > 1 && curr_id == 0) {
	for (li = 0; li < nloci; li++) {
		L[li].g_rec->upinf[IM_UPDATE_GENEALOGY_ANY].tries = 0;
		L[li].g_rec->upinf[IM_UPDATE_GENEALOGY_TOPOLOGY].tries = 0;
		L[li].g_rec->upinf[IM_UPDATE_GENEALOGY_TMRCA].tries = 0;
		L[li].g_rec->upinf[IM_UPDATE_GENEALOGY_ANY].accp = 0;
		L[li].g_rec->upinf[IM_UPDATE_GENEALOGY_TOPOLOGY].accp = 0;
		L[li].g_rec->upinf[IM_UPDATE_GENEALOGY_TMRCA].accp = 0;
	}
}
#endif
*/
 ...
 /* update population assignment */
  ...
  {/*
#ifdef DO_RY1UPDATE
#ifdef MPI_ENABLED
	if (curr_id == 0 && numprocesses > 1) {
		for (int j = 0; j < numsplittimes; j++) {
			T[j].upinf[IM_UPDATE_TIME_RY1].tries = 0;
			T[j].upinf[IM_UPDATE_TIME_RY1].accp = 0;
		}
	}
#endif
#endif
	...
	if (curr_id == 0 && numprocesses > 1) {
		for (int j  = 0; j < numsplittimes; j++) {
			T[j].upinf[IM_UPDATE_TIME_NW].tries = 0;
			T[j].upinf[IM_UPDATE_TIME_NW].accp = 0;
		}
	}
#endif
#endif
...
//	std::cout << "Chain ID " << current_id << "Returning " << (genealogiessaved < genealogiestosave) << "\n";
    ...
    case TIMEINF:
      ...
      {
	//std::cout << "Node " << current_id << " has now entered this stuff...\n";
        ...
        if (maxedoutgenealogysave)
	{
        ...
        {
	//AS:Fri Jan  8 11:30:50 EST 2016
	//Tomas Hrbek noted that there is a bug here. 
	//Trying to check if the file opening happens only one head node
	//std::cout << "Am I checking to see if IMrun is present? " << current_id << "\n";
	//std::cout << "System call\n";
	//system("ls; sync; ");
	//std::cout << "\n";
          if (current_id == 0 && (checkdonefile = fopen (imrunfile, "r")) == NULL)
          {
	//	std::cout << "IMrun is null so returning 0\n";
		continuerun = 0;
	  }
	//AS: here if there's more than 1 processor, this 0 has to be shared
	#ifdef MPI_ENABLED
		if (numprocesses > 1)	{
			if (current_id == 0) {
				for (int y = 1; y < numprocesses; y++)
					 MPI::COMM_WORLD.Send(&continuerun, 1, MPI::INT, y, y * 1234);
			}
			if (current_id != 0) {
				MPI::COMM_WORLD.Recv(&continuerun, 1, MPI::INT, 0, current_id * 1234);
			}			
		}	
	 #endif
	    if (continuerun == 0) {
          ...
          {
	//std::cout << "Am I checking inside IMrun now? " << current_id << "\n";
            if (current_id == 0) { 
            ch = (char) getc (checkdonefile);
	//	std::cout << "Ch here is " << ch << "\n";
	    fflush(checkdonefile);
	...
	//	std::cout << "continuerun here is " << continuerun << "\n";
	#endif
	if (continuerun == 0) {
		return(0);
	}
  ...
  else
  {
    ...
    if (runoptions[PRINTBURNTREND] && (tempburndone || step >= burntrendstartdelay))
    {
      ...
      if (tempburndone)
      {
        ...
        {
          if ((checkdonefile = fopen (imburnfile, "r")) != NULL)
          {
		std::cout << "IMburn file has been found...\n";
            ch = (char) getc (checkdonefile);
		std::cout << "ch is " << ch << "\n";
            f_close (checkdonefile);
            if ((char) toupper (ch) == 'Y')
	 ...
	 // else
	//	std::cout << "Can't open IMburn file...\n";
        }
      }
      else
      {
...
//JK 3/10/16: Does this need to be output as well?
...
//JK 3/9/16: Change return type, add TiXmlDocument declaration (acceptancerates)
TiXmlElement *callprintacceptancerates (FILE * outto, int currentid)
{
  int i, j, li;
...
int *z_rec = static_cast<int *> (malloc (numsplittimes * sizeof (int)));
 for (int x = 0; x < numsplittimes; x++) {
	...
	//	std::cout << "T[" << x << "].tries = " << T[x].upinf[IM_UPDATE_TIME_RY1].tries << "\n";
	//	std::cout << "T[" << x << "].accp = " << T[x].upinf[IM_UPDATE_TIME_RY1].accp << "\n";
		
	}
	}

//	std::cout << T[x].upinf[IM_UPDATE_TIME_RY1].tries << "\t" << T[x].upinf[IM_UPDATE_TIME_RY1].accp << "\n";
  XFREE(y);
  XFREE(y_rec);
  XFREE(z);
  XFREE(z_rec);
...
int *z_rec = static_cast<int *> (malloc (numsplittimes * sizeof (int)));
 ...
 XFREE(z_rec);
  ...
  {
    if (currentid == 0)
    acceptancerates->LinkEndChild(printacceptancerates (outto, numsplittimes, reclist,
                          "Update Rates -- Population Splitting Times")); //JK 3/9/16: link to acceptancerates
  }
...
for (int x = 0; x < nloci; x++) {
  ...
  if (currentid == 0)
	  acceptancerates->LinkEndChild(printacceptancerates (outto, nloci, reclist, "Update Rates -- Genealogies")); //JK 3/9/16: Link this to document
  
//  for (li = 0; li < nloci; li++)
//    L[li].g_rec = reclist_saved[li];
...
// kappa values for HKY model
    if (currentid == 0) 
    acceptancerates->LinkEndChild(printacceptancerates (outto, i, reclist,
                          "Update Rates -- Mutation Rate Scalars")); //JK 3/9/16: Link to acceptancerates
...
//AS: for HKY reduction 3/9/2016

    for (i = 0, li = 0; li < nloci; li++) {
      for (j = 0; j < L[li].nlinked; j++) {
	if (L[li].umodel[j] == HKY) {
	        reclist_saved[i] = &L[li].kappa_rec[j];
        	i++;
	      }
      }
     } //saving states
...
if (numprocesses > 1) {
 for (li = 0;  li < nloci; li++) {
    ...
    if (i > 0 && currentid == 0)
      acceptancerates->LinkEndChild(printacceptancerates (outto, i, reclist,
                            "Update Rates -- HKY Model Kappa parameter")); //JK 3/9/16: Link to acceptancerates
...
//8/26/2011  turn this printing section off, as it only ever prints zeros when A updating is not used 
  ...
  if (assignmentoptions[POPULATIONASSIGNMENT] == 1)
  {
    ...
    {
      acceptancerates->LinkEndChild(printacceptancerates (outto, nloci, reclist,
                            "Update Rates -- Assignment")); //Link to acceptancerates
    }
    // assignment updating information 
    // set reclest
    // call printacceptancerates 
    //
    printacceptancerates_multichain (outto); //link to acceptancerates
  }
...
//AS: Wed Dec  9 10:55:46 EST 2015
//AS: adding function to sort the thermosum array based on thermobetas array

int comparethermos (const void *a, const void *b) {
	double diff = thermobetas[*(int*)a] - thermobetas[*(int*)b];
	return (0 < diff) - (diff < 0);
}

//JK 3/9/16: declare XML doc, make filename, link calls, save document at end of function
void 
...
//JK 3/9/16: Add XML doc, create runbasics node, create filename
  char s[200];
  int n = sprintf(s,"%s.xml",outfilename);
  TiXmlDocument doc;
  TiXmlDeclaration *decl = new TiXmlDeclaration("1.0","","");
  doc.LinkEndChild(decl);
  TiXmlElement *runbasics = new TiXmlElement("RunBasics");
//AS: If doing thermodynamic integration, then the thermosums should be shared
//AS: Also, we need to tally up beta values, to correctly add up values according to intervals

#ifdef MPI_ENABLED
 if(calcoptions[CALCMARGINALLIKELIHOOD]) {
//	std::cout << "Before calling MPI Thermosums:" << "\n";
//	for (int y = 0; y < numchains * numprocesses; y++)
//		std::cout << thermosum[y] << "\t";
//	std::cout << "\n";	
	...
	for (int x = 1; x < numprocesses; x++) {
		if (currentid == x) {
			for (int v = 0; v < numchains; v++) {
				...
				MPI::COMM_WORLD.Send(&beta[v], 1, MPI::DOUBLE, 0, 9797);
			}
		}
		if (currentid  == 0) {
			for (int v = 0; v < numchains; v++) {
				MPI::COMM_WORLD.Recv(&thermosum[x * numchains + v], 1, MPI::DOUBLE, x, 9898);
				MPI::COMM_WORLD.Recv(&thermobetas[x * numchains + v], 1, MPI::DOUBLE, x, 9797);
			}
			//if (x == numprocesses -1) {
		//		std::cout << "After calling MPI from " << x << " Thermosums:" << "\n";
				//for (int y = 0; y < numchains * numprocesses; y++) {
		//			std::cout << thermosum[y] << "\t";
				//}
				//std::cout << "\n";
			//}
			for (int y = 0; y < numchains; y++) {
				thermobetas[y] = beta[y];
			}
			//for (int y = 0; y < numchains * numprocesses; y++) {
			//	std::cout << thermobetas[y] << "\t";
			//}
			//std::cout << "\n";	
		}
	}
	//AS: need to sort thermosum array based on thermobetas array
	int perm[numprocesses * numchains], res[numprocesses * numchains];
	if (currentid == 0) {
		for (i = 0; i != numchains * numprocesses; i++) {
			perm[i] = i;
		}
		qsort(perm, numchains * numprocesses, sizeof(int), comparethermos);
		//std::cout << "Thermobetas after sorting: \n";
		//for (i = 0; i != numchains * numprocesses; i++) {
		//	std::cout << thermobetas[perm[i]] << "\t";
		//}
		for (i  = 0; i != numchains * numprocesses; i++) {
			res[i] = thermosum[perm[i]];
		}
		//std::cout << "Thermosums before sorting: \n";
		//for (i = 0; i < numchains * numprocesses; i++)
		//	std::cout << thermosum[i] << "\t";
		//std::cout << "\n";

		//std::cout << "Thermosums after sorting: \n";
		int j = 0;
		for (i = numchains * numprocesses - 1; i != 0; i--) {
			thermosum[j] = res[i];
		//	std::cout << thermosum[j] << "\t";
			j++;
		}
		//std::cout << "\n";
	}
	//AS: now the thermosums should be sorted on the head node	
 }
#endif
...
if (currentid == 0) {
  ...
  runbasics->LinkEndChild(printrunbasics (outfile, runoptions[LOADRUN], fpstr, burnsteps, recordint,
                  recordstep, savegenealogyint, endtime, starttime, hilike,
                  hiprob/*, step*/));  //JK 3/9/16: Link to runbasics
  if (assignmentoptions[POPULATIONASSIGNMENT] == 1)
  {
  ...
  if (runoptions[LOADRUN] == 0)
  { //if (currentid == 0) {
    runbasics->LinkEndChild(callprintacceptancerates (outfile, currentid)); //JK 3/9/16: Link to runbasics
    //}
    if (numprocesses * numchains > 1 && runoptions[LOADRUN] != 1) { 
	      runbasics->LinkEndChild(printchaininfo (outfile, heatmode, hval1, hval2, currentid)); //JK 3/9/16: Link to runbasics
	}
	if (currentid == 0) {
   runbasics->LinkEndChild(printcurrentvals (outfile));
   callprintautoctable (outfile/*, step*/); //JK 3/10/16: Link this? 
    }
  }
if (currentid == 0) {
  ...
  if (!modeloptions[EXPOMIGRATIONPRIOR] && calcoptions[DONTCALCLIKELIHOODMUTATION]==0)        //as of 11/19/09 have not yet done the math for case of migration with exponential prior
    runbasics->LinkEndChild(print_means_variances_correlations (outfile));
...
/* get joint splittime peak */
  ...
  printhistograms (outfile, recordstep, generationtime, scaleumeaninput, outfilename);
  ...
  doc.SaveFile(s);
#endif
  return; //JK 3/9/16: Save XML document
}                               /* printoutput */

//JK 3/9/16: Make XML intervals file and filename
void intervaloutput (FILE * outto, int currentid)
{
  ...
  double multitpeak[MAXPOPS - 1];
  ci = 0;
//JK 3/14/16: Add XML doc declaration, intervals node
  char s[200];
  ...
  //AS: realized that this has to be done for the cold chain
  int z = whichiscoldchain();
  if (z >= 0) {
  ...
  #ifdef MPI_ENABLED
  if (z >=0 && currentid != 0) {
     MPI::COMM_WORLD.Send(&hilike, 1, MPI::DOUBLE, 0, 8787);
     MPI::COMM_WORLD.Send(&hiprob, 1, MPI::DOUBLE, 0, 6767);
     MPI::COMM_WORLD.Send(&like, 1, MPI::DOUBLE, 0, 5656);
  }
  if (z < 0 && currentid == 0) {
     MPI::COMM_WORLD.Recv(&hilike, 1, MPI::DOUBLE, MPI_ANY_SOURCE, 8787);
     MPI::COMM_WORLD.Recv(&hiprob, 1, MPI::DOUBLE, MPI_ANY_SOURCE, 6767);
     MPI::COMM_WORLD.Recv(&like, 1, MPI::DOUBLE, MPI_ANY_SOURCE, 5656);
  //AS: also these values need 
  //to be sent back to head node so they can be printed
  }
  #endif
      ...
      || outto != stdout)
  {
   if (currentid == 0)
    ...
    intervals->LinkEndChild(callprintacceptancerates (outto, currentid)); //JK 3/9/16: Link this, printcurrentvals, and printchaininfo to XML
   if (currentid == 0) {
    intervals->LinkEndChild(printcurrentvals (outto));
    callprintautoctable (outto /*, step*/);
    }
    if (numprocesses * numchains > 1)
      intervals->LinkEndChild(printchaininfo (outto, heatmode, hval1, hval2, currentid));
    if (currentid == 0) {
    if (genealogiessaved > 0)
    {
...
#ifdef XML_ENABLED
if(outto != stdout && currentid == 0)
  doc.SaveFile(s);
#endif
  return; //JK 3/9/16: Save doc
}                               /* intervaloutput */
...
void check_to_record (int current_id)
{
  ...
  if (i == recordint)
  {
    ...
    recordstep++;
    //AS: need to keep track of betas, for integration
    //for (int z = 0; z < numchains; z++)
	//std::cout << "Beta[i] = " << beta[z];
    i = 1;
  }
  else
  {
...
//JK 3/9/16: Set stdout buffer to 0 so front end doesn't get choppy results
int main (int argc, char *argv[])
{
  #ifdef MPI_ENABLED
  MPI::Init(argc, argv);
		...
		for (int x = 0; x < numprocesses; x++) {
			for (int y = 0; y < numprocesses; y++) {
				try {
					...
					std::cout << e.Get_error_string() << e.Get_error_code() << "\n";
					MPI::COMM_WORLD.Abort(Is that supposed to happen-1);
				}
			}
		}
