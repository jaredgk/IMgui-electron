                ...
                double hiprob/*, int step*/)
{
  ...
  //JK 3/10/16: Add highestpdg decl, set name attribute
  ...
  if (!loadrun == 1)
  {
    ...
    //if (z >= 0) {
    //JK 3/10/16: Add head node with "Locus  P(D|G)" text, link text to head and head to highestpdg
    TiXmlElement *head = new TiXmlElement("Head");
    TiXmlText *th = new TiXmlText("Locus  P(D|G)");
    head->LinkEndChild(th);
    highestpdg->LinkEndChild(head);
    TiXmlElement *data = new TiXmlElement("Data");
    for (li = 0; li < nloci; li++)
    {
      FP "\t%d\t%.3f\n", li, C[0]->G[li].hilike);
      //JK 3/10/16: Make stringstream, add li, set as attribute
      //Make text/row nodes, sstream hilike, link to highestpdg
      std::stringstream ss("");
      TiXmlElement *row = new TiXmlElement("Row");
      //row->SetAttribute("row",ss.str().c_str());
      ss << li << " " << C[0]->G[li].hilike;
...
//JK 3/10/16: Change return type
TiXmlElement *printacceptancerates (FILE * outto, int numrec,
                           ...
                           const char *printstring)
{
  ...
  int i, j;
  char numstr[20];
  //JK 3/10/16: acceptancerates decl, add printstring, header and subheader nodes
...
//JK 3/10/2016: Add stringstream, put upnames in and link to head node (add code later)
  ...
  fprintf (outto, "\n");
//JK 3/10/2016: Create text for subheader, link text to subhead and header nodes to acceptancerates, make data node
  ...
  for (j = 0; j < numrec; j++)
  {
    //JK 3/10/16: Reset sh stringstream, create row node and set rec->str as attr
    sh.str("");
    sh << "_" << rec[j]->str << "  ";
    TiXmlElement *row = new TiXmlElement("Row");
    ...
    {
      //JK 3/10/16: Create new stringstream, add try/accept/%, make text
      ...
      fprintf (outto, "\t%s", shorten_e_num (&numstr[0]));
      sh << shorten_e_num (&numstr[0]) << "  ";
      //Add to s based on if condition
...
//JK 3/10/16: Not finished in serial version
void printacceptancerates_multichain (FILE * outto)
{
  int ci;
  int i;
  char numstr[20];
  //JK 3/10/16: Add ac_multichain decl, link description text
/* TiXmlElement *ac_multichain = new TiXmlElement("UpdateRates_Multichain");
  TiXmlText *text = new TiXmlText("Update Rates -- Assignment per chain");
  ac_multichain->LinkEndChild(text); */
  ...
  for (i = 0; i < Cupinf[0].num_uptypes; i++)
    fprintf (outto, "\t#Tries\t#Accp\t%%");
  fprintf (outto, "\n");
  //JK 3/10/16: Add: TiXmlElement *data = new TiXmlElement("Data");
  for (ci = 0; ci < numchains; ci++)
  {
    //JK 3/10/16: Add stringstream, make row node, add cupinf to attr
    /* std::stringstream s("");
    s << "_" << Cupinf[ci].str;
    TiXmlElement *row = new TiXmlElement("Row");
    row->SetAttribute("row",s.str().c_str());
    s.str("");*/
    ...
    {
      //JK 3/10/2016: Put try/accept/% values in stringstream
...
//JK 3/10/16: Change return type
TiXmlElement *printcurrentvals (FILE * outto)
{
  int li, i;
  ...
  if (numsplittimes > 0)
  {
    ...
    fprintf (outto, "------------------------------\n");
    //JK: Make currentsplit, description, header, and link all
    TiXmlElement *currentsplit = new TiXmlElement("ValTable");
    TiXmlText *text = new TiXmlText("Current Splitting Time Values");
    currentsplit->LinkEndChild(text);
    TiXmlText *th = new TiXmlText("Time   Value");
    TiXmlElement *head = new TiXmlElement("Head");
    head->LinkEndChild(th);
    currentsplit->LinkEndChild(head);

    if (assignmentoptions[POPULATIONASSIGNMENTINFINITE] == 0
        && npops > 1)
    {
      //JK: make stringstream, data node, buffer for sprintf
      std::stringstream s("");
      TiXmlElement *data = new TiXmlElement("Data");
      char s_buf[50];
...
//JK: Add this in for loop: 
        sprintf (s_buf, "t%i",i);
        s << s_buf << "  ";
        TiXmlElement *row = new TiXmlElement("Row");
        //row->SetAttribute("row",s.str().c_str());
        //s.str("");
        sprintf(s_buf, "%.3f  ", C[0]->tvals[i]);
        s << s_buf;
      ...
      fprintf (outto, "\n\n");
//JK: make text, link to row and row to data
      //TiXmlText *t2 = new TiXmlText(s.str().c_str());
      //data->LinkEndChild(t2);
      currentsplit->LinkEndChild(data);

    }
    currentvals->LinkEndChild(currentsplit);
  }
  if (runoptions[PRINTMUTATIONUPDATESTOSCREEN] && nloci > 1)
  {
    ...
    fprintf (outto, "\n");
    //JK: Create nodes, header, description
    TiXmlElement *currentscalars = new TiXmlElement("ValTable");
    TiXmlText *text = new TiXmlText("Current Locus Likelihoods and Mutation Rate Scalars");
    currentscalars->LinkEndChild(text);
    TiXmlElement *head = new TiXmlElement("Head");
    TiXmlText *th = new TiXmlText("Locus#    p(D|G)     u  ");
    head->LinkEndChild(th);
    currentscalars->LinkEndChild(head);
    TiXmlElement *data = new TiXmlElement("Data");
    ...
    for (li = 0; li < nloci; li++)
    {
      for (i = 0; i < L[li].nlinked; i++)
      {
        ...
        std::stringstream s("");
        char s_buf[100];
        //JK: create row attribute (add code later)
        fprintf (outto, "%2i", li);
        s << li << "  ";
        if (L[li].nlinked > 1) {
          fprintf (outto, "_%-2d", i);
          sprintf (s_buf, "_%-2d", i);
          s << s_buf; 
        } else {
          fprintf (outto, "   ");
        }
        //row->SetAttribute("row",s.str().c_str());
        //s.str("");
        //JK: create data text (add code later)
        fprintf (outto, " %9.3lg %9.4lg", C[0]->G[li].pdg_a[i],
                 C[0]->G[li].uvals[i]);
        sprintf (s_buf, " %9.3lg %9.4lg", C[0]->G[li].pdg_a[i],
                 C[0]->G[li].uvals[i]);
        s << s_buf;
        TiXmlText *t2 = new TiXmlText(s.str().c_str());
        row->LinkEndChild(t2);
  ...
  //JK: Link data and scalars
    currentscalars->LinkEndChild(data);
    currentvals->LinkEndChild(currentscalars);
...
//JK 3/10/16: change return, need to see regular output to add XML
TiXmlElement *print_means_variances_correlations (FILE * outfile)
{
  double *means, *variances, **correlations;
  int i, p, q, np;
  TiXmlElement *pmvc = new TiXmlElement("MVC");
  TiXmlText *th = new TiXmlText("Means, Variances, and Correlations of parameters");
  pmvc->LinkEndChild(th);
  ...
  for (p = 0; p < np; p++)
  {
    ...
    else
    {
      if (imig[p - numpopsizeparams].pr.max > MPRIORMIN) {
          ...
          s << imig[p - numpopsizeparams].str << "  ";
      }
    }
  }
  TiXmlText *thead = new TiXmlText(s.str().c_str());
  head->LinkEndChild(thead);
  pmvc->LinkEndChild(head);
  TiXmlElement *data = new TiXmlElement("Data");
  s.str("");
  FP "\nMean:");
  s << "Mean  ";
  for (p = 0; p < np; p++) {
    FP "\t%-.3lf", means[p]);
    sprintf(s_buf, "%-.3lf", means[p]);
    s << s_buf << "  ";
  }
  TiXmlText *tr = new TiXmlText(s.str().c_str());
  TiXmlElement *r1 = new TiXmlElement("Row");
  r1->LinkEndChild(tr);
  data->LinkEndChild(r1);
  s.str("");
  FP "\nStdv:");
  s << "Stdev  ";
  for (p = 0; p < np; p++) {
    FP "\t%-.3lf", sqrt (variances[p]));
    sprintf(s_buf,"%-.3lf", sqrt (variances[p]));
    s << s_buf << "  ";
  }
  TiXmlText *ts = new TiXmlText(s.str().c_str());
  TiXmlElement *r2 = new TiXmlElement("Row");
  r2->LinkEndChild(ts);
  data->LinkEndChild(r2);
  s.str("");
  if (npops > 1)
  {
    FP "\n\nCorrelations\n");
    ...
    for (p = 0; p < np; p++)
    {
      ...
      {
        FP "%s", imig[p - numpopsizeparams].str);
        s << imig[p - numpopsizeparams].str << "-cor  ";
      }
      for (q = 0; q < np; q++)
      { 
        if (q == p) {
          ...
          if (q > p)
          {
            if (fabs (correlations[p][q]) < 0.4)
            {
              ...
              sprintf(s_buf, "%-.3lf", correlations[p][q]);
            }
            else
            {
              if (fabs (correlations[p][q]) < 0.75) {
                FP "\t%-.3lf%s", correlations[p][q], "$");
                sprintf(s_buf, "%-.3lf%s", correlations[p][q], "$");
              }
              else {
                FP "\t%-.3lf%s", correlations[p][q], "*");
                sprintf(s_buf, "%-.3lf%s", correlations[p][q], "*");
          ...
          else
          {
            if (fabs (correlations[q][p]) < 0.4)
            {
              ...
              sprintf(s_buf, "%-.3lf", correlations[q][p]);
            }
            else
            {
              if (fabs (correlations[q][p]) < 0.75) {
                FP "\t%-.3lf%s", correlations[q][p], "$");
                sprintf(s_buf, "%-.3lf%s", correlations[q][p], "$");
              } else {
                FP "\t%-.3lf%s", correlations[q][p], "*");
                sprintf(s_buf, "%-.3lf%s", correlations[q][p], "*");
              }
