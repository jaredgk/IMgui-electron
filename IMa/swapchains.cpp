...
setheat (double hval1, double hval2, int heatmode, int currentid)
{
  ...
  int x = 0;
  double h = 0.0;
  int hchains = numprocesses * numchains;
   /* 5/19/2011 JH adding thermodynamic integration */
...
//	if (currentid == 0) {
		...
		for (int i = 0; i < numprocesses * numchains; i++) {
				...
				case HGEOMETRIC:
					...
					break;
				case HEVEN:
					/*if (ODD(numprocesses * numchains) == 0) {
						hchains = numprocesses * numchains - 1;
					} else {
						hchains = numprocesses * numchains;
					}*/
					h = 1.0 / (hchains - 1);
					//std::cout << "h = " << h << "\n";
					//if (i == 0) {
					...
					//	} else {
							allbetas[x] = 1.0 - i * h;	
							//std::cout << "allbetas[x] = " << allbetas[x] << "\n";
					//	}
						x++;
						break;
					//}
				...
				if (i == hchains)
					break;
		//	std::cout << "allbetas[i] = " << allbetas[i] << "\n";
		}
...
/*  9/26/2011 thermodynamic integration handled by simpson's rule which requires an even number of intervals 
  ...
  //if (heatmode == HEVEN  && ODD(numprocesses * numchains) == 0 /*&& numprocesses == 1*/)
//	hchains = numchains - 1;
  // }
  ...
  for (ci = currentid * numchains; ci < currentid * numchains + numchains; ci++)
  {
   ...
   /* case HTWOSTEP: stopped using 8/23/2011 */ 
	...
	//}
      //else
      //{
       // if (currentid == (numprocesses - 1) && ci == hchains) {
        //  beta[i] = 0.0;
	//betaref1[i] = beta[i];
	//betaref2[i] = beta[i];
	//break;
			...
			int burnduration,/* std::ofstream &f1, int swapA, int swapB,*/ int heatmode)
{
	...
	int p = 0;
	int q = 0;
        //AS: Tue Mar  8 15:47:33 EST 2016
	int l = 0;
        int sa = 0;
        int sb = 0;
        int x = 0;
        ...
        int sbmin, sbrange;
        #define SWAPDIST 7
	//AS: Tue Mar  8 15:49:10 EST 2016
	for (x = 0; x < swaptries; x++) {
		...
		//std::cout << "on processor " << current_id << " sa = " << sa << " sb = " << sb << "\n";
		abeta = allbetas[sa];
		bbeta = allbetas[sb];
		//std::cout << "on processor " << current_id << " abeta = " << abeta << " bbeta = " << bbeta << "\n";
		whichElementA = 0;
		whichElementB = 0;
		doISwap = 0;
		areWeA = 0;
		...
		for (y = 0; y < numchains; y++) {
			if (beta[y] == abeta) {
				...
				areWeA = 1;
				whichElementA = y;
		//		std::cout << "whichElementA = " << whichElementA << "\n";
		...
		for (y = 0; y < numchains; y++) {
			if (beta[y] == bbeta) {
				...
				whichElementB = y;
		//		std::cout << "WhichElementB = " << whichElementB << "\n";
				doISwap = 1;
				if (current_id != 0) {
...
*/
	...
	//	doISwap = 1;
	//}
	/*if (doISwap == 0 && heatmode == HEVEN) {
		for (int y = 0; y < numchains; y++)
			tempthermosums[y] = 0.0;
	}*/
	if (doISwap == 1) {
	//std::cout << "Am i here at all?!?! on proc " << current_id << "\n"; 
	
	if (procIdForA == procIdForB && procIdForA == current_id) {
	//	f1 << "Pushing this to within process swap...\n Run done? " << rundone;
		swapvar = swapchains(1, swapbetasonly, current_id, heatmode);
...
//		swapcount_bwprocesses[currentid * numchains + ci][currentid * numchains + cj]++;
	...
	if (swapbetasonly) {
		swapbetas(ci, cj);
		//AS: Adding HEVEN case, where thermosums have to be updated too according to swap
		/*if (heatmode == HEVEN) {
			for (int y = 0; y < numchains; y++) {
				tempthermosums[y] = C[y]->allpcalc.pdg;
			}
			tempthermosums[cj] = C[ci]->allpcalc.pdg;
			tempthermosums[ci] = C[cj]->allpcalc.pdg;
			
			//std::cout << "Thermosums on processor " << currentid << "\n";
			//for (int y = 0; y < numchains; y++)
			//	std::cout << tempthermosums[y] << "\t";
			//std::cout << "\n";
		}*/
...
//JK 3/10/16: change return, code XML later
TiXmlElement *
printchaininfo (FILE * outto, int heatmode, double hval1,
                double hval2, int currentid)
{
  TiXmlElement *swaprates = new TiXmlElement("SwapRates");
  int i;
  if (currentid == 0) {
  fprintf (outto, "\nCHAIN SWAPPING BETWEEN SUCCESSIVE TEMPERATURES: ");
  switch (heatmode)
...
//AS: commenting this out to print swaps between adjacent temps instead
/*fprintf (outto, "beta terms :");
  ...
  else
  {
    ...
    TiXmlElement *st1 = new TiXmlElement("SwapTable");
    TiXmlText *t1 = new TiXmlText("Chain Swapping Between Successive Temperatures"); 
    st1->LinkEndChild(t1);
    TiXmlElement *head1 = new TiXmlElement("Head");
    TiXmlText *th = new TiXmlText("Chain  #Swaps  Rate");
    head1->LinkEndChild(th);
    st1->LinkEndChild(head1);
  ...
  //    if (swapcount_bwprocesses[i + 1][i])
        ...
        std::stringstream s("");
        s << i << "  " << swapcount_bwprocesses[i][i+1] << "  " << (swapcount_bwprocesses[i][i + 1] / (float) swapcount_bwprocesses[i + 1][i]);
        TiXmlText *t = new TiXmlText(s.str().c_str());
        row->LinkEndChild(t);
    ...
    st1->LinkEndChild(data);
    swaprates->LinkEndChild(st1);
  }
  //AS: 3/18/2016 - TODO:Very important - add swapcount printing if 
  // numprocesses == 1; currently only printing swap rates if > 1
  if (numprocesses > 1 && currentid == 0) {
	...
	fprintf (outto, "Processor1  Processor2  #SwapAttempts\n");
        TiXmlElement *st2 = new TiXmlElement("SwapTable");
        TiXmlText *t2 = new TiXmlText("Chain Swapping Attempts Between Processors");
        st2->LinkEndChild(t2);
        TiXmlElement *h2 = new TiXmlElement("Head");
        TiXmlText *th2 = new TiXmlText("Processor1  Processor2  #SwapAttempts");
        h2->LinkEndChild(th2);
        st2->LinkEndChild(h2);
		...
		for (int j = 0; j < numprocesses; j++) {
			if (i > j) {
				fprintf (outto, " %3d      %3d     %5ld\n", i, j, swaps_rec_bwprocesses[i][j]);
                                TiXmlElement *row = new TiXmlElement("Row");
                                s << i << "  " << j << "  " << swaps_rec_bwprocesses[i][j];
                                TiXmlText *t = new TiXmlText(s.str().c_str());
                                row->LinkEndChild(t);
                                data2->LinkEndChild(row);
                                s.str("");
			}
		}
	}
        st2->LinkEndChild(data2);
        swaprates->LinkEndChild(st2);
	fprintf (outto, "\n\n");

        TiXmlElement *st3 = new TiXmlElement("SwapTable");
        TiXmlText *t3 = new TiXmlText("Chain Swapping Between Processors");
        st3->LinkEndChild(t3);
        TiXmlElement *h3 = new TiXmlElement("Head");
        TiXmlText *th3 = new TiXmlText("Processor1  Processor2  #Swaps");
        h3->LinkEndChild(th3);
        st3->LinkEndChild(h3);
		...
		for (int j = 0; j < numprocesses; j++) {
			if (i < j) {
				fprintf (outto, " %3d      %3d      %5ld\n", i, j, swaps_rec_bwprocesses[i][j]);
                                ...
                                TiXmlText *t = new TiXmlText(s.str().c_str());
                                row->LinkEndChild(t);
                                data3->LinkEndChild(row);
                                s.str("");
			}
		}
	}
        st3->LinkEndChild(data3);
        swaprates->LinkEndChild(st3);
	fprintf (outto, "\n\n");
	...
	for (i = 0; i < numprocesses * numchains; i++) {
        ...
        TiXmlElement *st4 = new TiXmlElement("SwapTable");
        TiXmlText *t4 = new TiXmlText("Chain Swapping Between Temperatures");
        st4->LinkEndChild(t4);
        TiXmlElement *h4 = new TiXmlElement("Head");
        TiXmlText *th4 = new TiXmlText("Temp1  Temp2  #Swaps  #Attempts  Rate");
        h4->LinkEndChild(th4);
        st4->LinkEndChild(h4);
	...
	for (i = 0; i < numprocesses * numchains - 1; i++) {
		//for (int j = 0; j < numprocesses * numchains; j++) {
                        s.str("");
                        TiXmlElement *row = new TiXmlElement("Row");
                        s << allbetas[i] << "  " << allbetas[i+1] << "  " << tempbased_rec_swapcount[i][i+1] << "  " << tempbased_rec_swapcount[i+1][i] << "  ";
			if (/*i < j && */tempbased_rec_swapcount[i+1][i] > 0) {
				fprintf (outto, " %7.4f    %7.4f   %5ld   %5ld   %7.4f\n", allbetas[i], allbetas[i+1], tempbased_rec_swapcount[i][i+1], tempbased_rec_swapcount[i+1][i], (float) tempbased_rec_swapcount[i][i+1]/ (float) tempbased_rec_swapcount[i+1][i]);
                                s << ((float) tempbased_rec_swapcount[i][i+1]/ (float) tempbased_rec_swapcount[i+1][i]);
			}
			else if (/*i < j && */tempbased_rec_swapcount[i+1][i] == 0) {
				fprintf (outto, " %7.4f    %7.4f   %5ld   %5ld   na\n", allbetas[i], allbetas[i+1], tempbased_rec_swapcount[i][i+1], tempbased_rec_swapcount[i+1][i]);
                                s << "na";
        ...
        st4->LinkEndChild(data4);
        swaprates->LinkEndChild(st4);
	fprintf (outto, "\n\n");
	}
	}
  return swaprates;
