...
static TiXmlElement *writehistogram (FILE * outfile, int numhistprint, int dosmooth, const char * printstring); //JK 3/10/16: change return type, add printstring arg
...
static TiXmlElement *print_tprior_divide_histograms (FILE * outfile,
                                            int *numhistprint); //JK 3/10/16: change return type?
...
*/
//JK 3/10/16: Change return type
TiXmlElement *
writehistogram (FILE * outfile, int numhistprint, int dosmooth, const char * printstring)
{
  double *xysum;
  double *ysum;
  double *xsum;
  ...
  smthprobvals = static_cast<double *> 
                 (calloc ((size_t) numhistprint, sizeof (double)));
  //JK: declare xml histogram node, summary node, stringstream
  TiXmlElement *histogram = new TiXmlElement("Histogram"); 
  TiXmlElement *row1 = new TiXmlElement("Summaries");
  row1->SetAttribute("description","Summaries of all parameters");
  std::stringstream s("");
  ...
  //JK: declare minimum bun row and values, link previous row, clear stringstream
  TiXmlText *text = new TiXmlText(s.str().c_str());
  row1->LinkEndChild(text);
  histogram->SetAttribute("description",printstring);
  histogram->LinkEndChild(row1);
  TiXmlElement *row2 = new TiXmlElement("Minbin");
  row2->SetAttribute("description","Minimum bin");
  s.str("");
  ...
  for (j = 0; j < numhistprint; j++)
  {
    ...
    s << histformatdouble (hp[j].xscaleadjust * hp[j].xy[i].x);
  }
//JK: Link minbin to node, make row 3 and attribute
  TiXmlText *text1 = new TiXmlText(s.str().c_str());
  row2->LinkEndChild(text1);
  histogram->LinkEndChild(row2);
  TiXmlElement *row3 = new TiXmlElement("Maxbin");
  row3->SetAttribute("description","Maximum bin");
  s.str("");
...
//JK: Link previous row, create nodes for next row
  TiXmlText *text2 = new TiXmlText(s.str().c_str());
  row3->LinkEndChild(text2);
  histogram->LinkEndChild(row3);
  TiXmlElement *row4 = new TiXmlElement("HiPt");
  row4->SetAttribute("description","Highest Point");
  s.str("");
  ...
  for (j = 0; j < numhistprint; j++)
  {
    ...
    JK: s << histformatdouble (hp[j].xscaleadjust * hp[j].xy[imax].x) << "  ";

  }
//JK: Link text and row
  TiXmlText *text5 = new TiXmlText(s.str().c_str());
  row4->LinkEndChild(text5);
  histogram->LinkEndChild(row4);
  s.str("");
  if (dosmooth)
  {
    FP "\n\tHiSmth");
    //JK: Start row, set attribute, clear ss
    TiXmlElement *row5 = new TiXmlElement("HiSmth");
    row5->SetAttribute("description","Highest point after smoothing");
    ...
    for (j = 0; j < numhistprint; j++)
    {
      ...
      s << histformatdouble (smthmaxvals[j]) << "  ";
    }
//JK: link to histogram
    TiXmlText *text6 = new TiXmlText(s.str().c_str());
    row5->LinkEndChild(text6);
    histogram->LinkEndChild(row5);

  }
//JK: make node for next row, set attrbute, clear ss
  TiXmlElement *row6 = new TiXmlElement("Mean");
  row6->SetAttribute("description","Mean value");
  s.str("");
  ...
  for (j = 0; j < numhistprint; j++)
  {
    ...
    s << histformatdouble (xysum[j] / ysum[j]) << "  ";
  }
//JK: Link previous row to hist, init next row, clear ss
  TiXmlText *text7 = new TiXmlText(s.str().c_str());
  row6->LinkEndChild(text7);
  histogram->LinkEndChild(row6);
  TiXmlElement *row7 = new TiXmlElement("_95Lo");
  row7->SetAttribute("description","Lowest 95 percent");
  s.str("");
...
//JK: Link prev row, init next row, reset ss
  TiXmlText *text8 = new TiXmlText(s.str().c_str());
  row7->LinkEndChild(text8);
  histogram->LinkEndChild(row7);
  TiXmlElement *row8 = new TiXmlElement("_95Hi");
  row8->SetAttribute("description","Highest 95 percent");
  s.str("");
  FP "\n\t95%%Hi  ");
  for (j = 0; j < numhistprint; j++)
  {
...
//JK: Link prev row
  TiXmlText *text9 = new TiXmlText(s.str().c_str());
  row8->LinkEndChild(text9);
  histogram->LinkEndChild(row8);
  s.str("");
...
//JK: Create next row/attribute
TiXmlElement *row9 = new TiXmlElement("HPDLo");
  row9->SetAttribute("description","HPD low");

  FP "\n\tHPD%sLo",hpdstr);
  for (j = 0; j < numhistprint; j++)
  {
    FP "\t%s%c%c", histformatdouble (hpdlo[j]), hpdchar1[j], hpdchar2[j]);
    s << histformatdouble (hpdlo[j]) << hpdchar1[j] << hpdchar2[j] << "  ";
  }
//JK: Link prev, init next, clear ss
  TiXmlText *text10 = new TiXmlText(s.str().c_str());
  row9->LinkEndChild(text10);
  histogram->LinkEndChild(row9);
  TiXmlElement *row10 = new TiXmlElement("HPDHi");
  row10->SetAttribute("description","HPD High");
  s.str("");
  ...
  for (j = 0; j < numhistprint; j++)
  {
    ...
    s << histformatdouble(hpdhi[j]) << hpdchar1[j] << hpdchar2[j] << "  ";
  }
//JK: link prev row, init next row, clear ss
  TiXmlText *text11 = new TiXmlText(s.str().c_str());
  row10->LinkEndChild(text11);
  histogram->LinkEndChild(row10);
  TiXmlElement *row11 = new TiXmlElement("Parameter");
  row11->SetAttribute("description", "Parameter");
  s.str("");
...
//JK:
//link prev row, init next row
  TiXmlText *text12 = new TiXmlText(s.str().c_str());
  row11->LinkEndChild(text12);
  histogram->LinkEndChild(row11);

  TiXmlElement *row12 = new TiXmlElement("HiPt");
  row12->SetAttribute("description","HighPt");
  s.str("");
  ...
  for (j = 0; j < numhistprint; j++)
  {
    ...
    FP "\t%s", histformatdouble (hp[j].xscaleadjust * hp[j].xy[imax].x));
    s << histformatdouble (hp[j].xscaleadjust * hp[j].xy[imax].x) << "  ";
    FP "\t%s", histformatdouble (hp[j].yscaleadjust * hp[j].xy[imax].y));
    s << histformatdouble (hp[j].yscaleadjust * hp[j].xy[imax].y) << "  ";
  }
  FP "\n\n");
//JK: link prev row, create data node
  TiXmlText *text13 = new TiXmlText(s.str().c_str());
  row12->LinkEndChild(text13);
  histogram->LinkEndChild(row12);
  TiXmlElement *data = new TiXmlElement("Data");

  for (i = 0; i < GRIDSIZE; i++)
  {
//JK: init row (does it need locus number?)
    ...
    for (j = 0; j < numhistprint; j++)
    {
      ...
      s << histformatdouble (hp[j].yscaleadjust * hp[j].xy[i].y) << "  ";
    }
    FP "\n");
//JK: link text to row, row to data
    TiXmlText *text14 = new TiXmlText(s.str().c_str());
    row->LinkEndChild(text14);
    data->LinkEndChild(row);

  }
//JK: link data, create summary row
  histogram->LinkEndChild(data);
  TiXmlElement *row13 = new TiXmlElement("SumP");
  row13->SetAttribute("description", "Sum P");
  s.str("");

  FP " SumP\t");
  for (j = 0; j < numhistprint; j++) {
    FP "\t\t%s", histformatdouble (ysum[j]));
    s <<  histformatdouble(ysum[j]) << "  ";
  }
  FP "\n");
//JK: Link prev row, init next, reset ss
  TiXmlText *text15 = new TiXmlText(s.str().c_str());
  row13->LinkEndChild(text15);
  histogram->LinkEndChild(row13);
  TiXmlElement *row14 = new TiXmlElement("Before");
  row14->SetAttribute("description","Before");
  s.str("");

  FP " Before\t");
  for (j = 0; j < numhistprint; j++) {
    FP "\t\t%s", histformatdouble (hp[j].before * hp[j].yscaleadjust));
    s  << histformatdouble (hp[j].before * hp[j].yscaleadjust) <<  "  ";
  }
//link, init, reset
  TiXmlText *text16 = new TiXmlText(s.str().c_str());
  row14->LinkEndChild(text16);
  histogram->LinkEndChild(row14);

  TiXmlElement *row15 = new TiXmlElement("After");
  row15->SetAttribute("description","After");
  s.str("");
  ...
  for (j = 0; j < numhistprint; j++) {
    ...
    s << histformatdouble (hp[j].after * hp[j].yscaleadjust) << "  ";
  }
//Link last row
  TiXmlText *text17 = new TiXmlText(s.str().c_str());
  row15->LinkEndChild(text17);
  histogram->LinkEndChild(row15);
...
//JK: change return type
TiXmlElement *print_tprior_divide_histograms (FILE * outfile, int *numhistprint)
{
  ...
  //writehistogram (outfile, *numhistprint,1); //JK: comment this out
  orig2d_free2D ((void **) t_prior_divide, numsplittimes); //JK 3/15/16: Does this need to be moved into the writehistogram function?
  return (writehistogram(outfile, *numhistprint,1,"Divergence Time/Mutation Rates"));
}                               //void prepare_tprior_divide_histograms(int *numhistprint);


//JK 3/10/16: change return type
TiXmlElement *print_populationmigrationrate_histograms (FILE * outfile,
                                               int *numhistprint,
                                               int prob_or_like)
{
  ...
  //writehistogram (outfile, hpi,0); //JK: comment this line out
  orig2d_free2D ((void **) popmigxy, *numhistprint);
  return(writehistogram(outfile, hpi, 0,"Population Sizes and Migration Rates"));
...
//JK: init doc, declaration, make histogram node
  ...
  if (numhistprint)
  {
    ...
    allhistograms->LinkEndChild(writehistogram (outfile, numhistprint,1,"Temporary Name")); //JK: link to allhistograms
  ...
  if (numhistprint)
  {
    ...
    FP "--------------------------------------------------------------------------------------------------------\n");
    FP "       curve height is an estimate of marginal posterior probability\n");
    allhistograms->LinkEndChild(writehistogram (outfile, numhistprint,0,"Temporary Name")); //JK: link to allhistograms
  }
  numhistprint = 0;
...
//JK: link next statement to doc
  ...
  if (outputoptions[THISTDIVIDEBYPRIOR] && numsplittimes > 1)
  {
    ...
    numhistprint = 0;
//JK: link next statement to allhistograms
    allhistograms->LinkEndChild(print_tprior_divide_histograms (outfile, &numhistprint));    // writehistogram() called from within this because of memory allocation within 
  }
  free_print_histogram ();
//JK: link allhistograms, create filename, save to file
  doc.LinkEndChild(allhistograms);
  std::stringstream s("");
  s << outfilename << ".histograms.xml";
#ifdef XML_ENABLED
  doc.SaveFile(s.str().c_str());
#endif
}                               // printhistograms
...
//JK: any additional changes to this?
...
/*  FP "     Curves labeled by source and target population and " "_t"
  ...
  for (i = 0; i < nloci + (nloci > 1); i++)
  {
    ...
    FP "===================\n");
    /* 8/26/2011 */
    doc.LinkEndChild(writehistogram (outfile, nummigdirs,1,"Temporary Name")); //JK: link to dov
    //writehistogram (outfile, 2 * nummigdirs,1);
  }
#ifdef XML_ENABLED
  doc.SaveFile("mighistograms.xml");
#endif
  free_print_histogram ();
